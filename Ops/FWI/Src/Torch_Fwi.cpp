...
std::vector<torch::Tensor> fwi_forward(torch::Tensor &th_Lambda,
                                       torch::Tensor &th_Mu,
                                       torch::Tensor &th_Den,
                                       torch::Tensor &th_stf, int gpu_id,
                                       torch::Tensor &th_shot_ids,
  ...
  cufd(misfit_ptr, NULL, NULL, NULL, NULL, Lambda, Mu, Den, stf, 0, gpu_id,
       group_size, shot_ids, para_fname);
  torch::Tensor th_misfit = torch::from_blob(misfit_ptr, {1});
  return {th_misfit.clone()};
}

std::vector<torch::Tensor> fwi_backward(torch::Tensor th_Lambda,
                                        torch::Tensor th_Mu,
                                        torch::Tensor th_Den,
                                        torch::Tensor th_stf, int ngpu,
                                        torch::Tensor th_shot_ids,
                                        const string para_fname) {
  ...
  std::vector<torch::Tensor> vec_misfit(ngpu);
...
#pragma omp parallel for num_threads(ngpu)
  ...
  for (int i = 0; i < ngpu; i++) {
    ...
    th_misfit_sum += vec_misfit.at(i);
  }
  return {th_misfit_sum.clone(), th_grad_Lambda_sum, th_grad_Mu_sum,
          th_grad_Den_sum, vec_grad_stf.at(0)};
}

void fwi_obscalc(torch::Tensor th_Lambda, torch::Tensor th_Mu,
                 torch::Tensor th_Den, torch::Tensor th_stf, int ngpu,
                 torch::Tensor th_shot_ids, const string para_fname) {
  const int group_size = th_shot_ids.size(0);
  if (ngpu > group_size) {
    printf("The number of GPUs should be smaller than the number of shots!\n");
    exit(1);
...
#pragma omp parallel for num_threads(ngpu)
  for (int i = 0; i < ngpu; i++) {
    ...
    auto th_sub_shot_ids = th_shot_ids.narrow(0, startBar, endBar - startBar);
    auto shot_ids = th_sub_shot_ids.data_ptr<int>();
    cufd(NULL, NULL, NULL, NULL, NULL, Lambda, Mu, Den, stf, 2, i,
         th_sub_shot_ids.size(0), shot_ids, para_fname);
